    ...
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            if data_dict.get("fast_skip"):
                ...
                logger.info(
                    "Test: [{}/{}] {} {:.2f}s (cached)".format(
                        idx + 1, len(self.test_loader), data_name, batch_time.val
                    )
                )
                continue
        ...
        comm.synchronize()
        intersection_meter_all = comm.all_reduce(intersection_meter.sum)
        union_meter_all = comm.all_reduce(union_meter.sum)
        target_meter_all = comm.all_reduce(target_meter.sum)

        class_iou = intersection_meter_all / (union_meter_all + 1e-10)
        class_acc = intersection_meter_all / (target_meter_all + 1e-10)
        mAcc = np.mean(class_acc[target_meter_all > 0])
        allAcc = sum(intersection_meter_all) / (sum(target_meter_all) + 1e-10)
        mIoU = np.mean(class_iou[target_meter_all > 0])

        logger.info("Val result: mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}.".format(mIoU, mAcc, allAcc))
        for i in range(self.cfg.data.num_classes):
            logger.info(
                "Class_{}-{} Result: iou/accuracy {:.4f}/{:.4f}".format(
                    i, self.cfg.data.names[i], class_iou[i], class_acc[i]
                )
            )
