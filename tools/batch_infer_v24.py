#!/usr/bin/env python
"""V24 batch inference — Base model + grid_size=0.025"""
...
GRID_SIZE = 0.025
OUT_ROOT = "/root/autodl-tmp/exp/forest/semseg-litept-base-v1m1-loss-v2-clean-v24/inference"
...
class V24Inference(_Inference):
    def _build_model(self):
        ...
        print(f"Loading V24 Base weights: {self.weight_path}", flush=True)
        ...
        print(f"V24 Base model epoch {ckpt['epoch']} | params: {nt:,} | {self.device}", flush=True)


def main():
    os.makedirs(OUT_ROOT, exist_ok=True)
    print(f"V24 Batch Inference", flush=True)
